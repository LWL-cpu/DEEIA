...
def hungarian_matcher(predicted_spans, target_spans):
    ...
    cost_spans = torch.cdist(torch.FloatTensor(predicted_spans).unsqueeze(0),
                             torch.FloatTensor(target_spans).unsqueeze(0), p=1)
    ...
    def generate_mask(feature):
        ...
        for start in range(context_length):
            ...
            if start_index < args.max_enc_seq_length:
                end_index_list = [feature.old_tok_to_new_tok_index[end - 1][1] for end in
                                  range(start + 1, min(context_length, start + args.max_span_length + 1))
...
def filter_spans(candidate_span_list, max_span_num):
    ...
    if len(candidate_span_list) == 1:
        ...
        while candidate_span_list and len(answer_span_list) < max_span_num:
            ...
            candidate_span_list = [candidate_span for candidate_span in candidate_span_list if
                                   not is_intersect(candidate_span, selected_span)]
